 ...
 * @author annamartin
 */
public class SimpleMlpBatch implements SimpleMlp {
    ...
    private double m_learningRate = 0.01; //learning rate
     ...
     * @param lrdecay
     */
    public SimpleMlpBatch(final int nInput, final int nHidden, final int nOutput, final Random random,
                          final double learnrate, final Boolean shuffle, final int lrdecayrate, final boolean lrdecay) {
     ...
     * @param outputsarr
     */
    public void backPropagate(final Double[] outputsarr) {
    ...
    public void trainNetwork(final Double[][] samplesarr, final Double[][] outputsarr, final int iteration) {
        ...
        //set delta arrays to 0

        for (int i = 0; i < m_deltaL1.length; i++) {
            for (int j = 0; j < m_deltaL1[0].length; j++) {
                m_deltaL1[i][j] = 0.0;
            }
        }

        for (int i = 0; i < m_deltaL2.length; i++) {
            for (int j = 0; j < m_deltaL2[0].length; j++) {
                m_deltaL2[i][j] = 0.0;
            }
        }
        ...
        for (int i = 0; i < samplesarr.length; i++) {
            if (m_shuffle) {
                ...
                backPropagate(outputsarr[indexArray.get(i)]);
            } else {
                passExample(samplesarr[i]);
                backPropagate(outputsarr[i]);
            }
        }
        ...
        // finally update weights on the 2nd layer
        for (int i = 0; i < m_weightsL2.length; i++) {
            for (int j = 0; j < m_weightsL2[0].length; j++) {
                m_weightsL2[i][j] = m_weightsL2[i][j] + m_deltaL2[i][j] * (m_learningRate / (1 + iteration * d));
            }
        }

        // finally update weights on the 1st layer
        for (int i = 0; i < m_weightsL1.length; i++) {
            for (int j = 0; j < m_weightsL1[0].length; j++) {
                m_weightsL1[i][j] = m_weightsL1[i][j] + m_deltaL1[i][j] * (m_learningRate / (1 + iteration * d));
            }
        }
